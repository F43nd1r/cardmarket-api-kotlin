...
class ShoppingCartService(config: CardmarketApiConfiguration) : AbstractService(config) {
    ...
    fun removeArticles(vararg articles: Article): ShoppingCart? =
            put("shoppingcart").body {
    ...
    fun emptyShoppingCart() : Unit = delete("shoppingcart").submit() ?: Unit

    fun checkout(): List<Order> = put("shoppingcart/checkout").submit("order") ?: emptyList()
    ...
    fun getShippingMethods(reservation: Reservation): List<ShippingMethod> = get("shoppingcart/shippingmethod/${reservation.idReservation}").submit("availableMethod") ?: emptyList()
